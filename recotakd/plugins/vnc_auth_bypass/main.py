...
def test_vnc_authentication_bypass(ip, port, timeout, verbose):
...
def main(dbh, target=None, port=None, timeout=60, verbose=False, **kwargs):
    ...
    # status 4 = bad connection
    # status 5 = too many failures
    status = test_vnc_authentication_bypass(target, port, timeout, verbose)
    if status == 0:
...
desc = {
    ...
    "plgname": "vnc_auth_bypass",

    "plgauthor": "dash",

    "desc": "vnc authentication bypass",
